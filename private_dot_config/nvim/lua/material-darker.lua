...
local M = {}
local base_opts
local loading_material = false

function M.setup(opts)
  base_opts = vim.deepcopy(opts)
  -- When our entry point lazy-loads Catppuccin, load() sets it up once with
  -- Material already applied. Don't compile stock colours only to discard them.
  if not loading_material then
    require("catppuccin").setup(opts)
  end
  vim.api.nvim_create_autocmd("ColorSchemePre", {
    group = vim.api.nvim_create_augroup("material_darker", { clear = true }),
    callback = function(ev)
      -- Catppuccin has one shared configuration. Restore it before loading any
      -- real flavour; leave it alone for unrelated themes (as before extraction).
      if ev.match:match("^catppuccin") then
        require("catppuccin").setup(vim.deepcopy(base_opts))
      end
      if vim.g.colors_name == "material-darker" and ev.match ~= "material-darker" then
        for i = 0, 15 do
...
function M.load()
  ...
  local catppuccin = require("catppuccin") -- also loads LazyVim's merged integrations
  ...
  catppuccin.setup(vim.tbl_deep_extend("force", vim.deepcopy(base_opts), opts))
  -- Call the engine directly: :colorscheme here would emit a second lifecycle.
  catppuccin.load("macchiato")
  vim.g.colors_name = "material-darker"
  for i = 0, 15 do
    vim.g["terminal_color_" .. i] = material_terminal[i]
  end
end
